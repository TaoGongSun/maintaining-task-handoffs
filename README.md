...
**何時不寫**：純問答、沒有後續價值的短任務。  
**聊天怎麼回**：只保留交接文件路徑、精簡狀況，以及最後一行的**唯一具體下一步**；細節都在 `.ai/HANDOFF.md`。
...
`handoff pause` 表示這一輪已安全收尾，但 HANDOFF 的整體 Goal 仍有後續工作；它接受 `in-progress` 或 `blocked`，保留具體 Next action，也不封存計畫。`handoff complete` 只用於整體 Goal 確實完成的情況。兩者都會解除當前 active lifecycle，之後可用同一 task id 再次 checkpoint 來接續 paused 工作。
...
| 整體 Goal 完成 | `handoff complete`；只有此時可封存明列計畫 |
...
**Chat after writing:** only the file path, a concise status, and a **single concrete next action** on the last line; detail stays in `.ai/HANDOFF.md`.
...
`handoff pause` safely ends the current run while the HANDOFF's overall Goal still has remaining work. It accepts `in-progress` or `blocked`, preserves a concrete Next action, and never archives plans. `handoff complete` is reserved for a genuinely completed Goal. Both release the active lifecycle; a later checkpoint with the same task id resumes paused work.
...
| Overall Goal completed | `handoff complete`; only this may archive listed plans |
