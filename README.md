...
若當前任務有計畫文件，可在草稿加入可選的 `## Plan files`，以 Markdown bullet 明列該任務擁有的 repo-relative paths。`handoff complete` 只封存這份 HANDOFF 明列的計畫，絕不掃描目錄或整理未列出的文件。一般計畫移至同目錄的 `archive/<year>/`；`.ai/plans/` 下的計畫移至 `.ai/archive/plans/<year>/`。來源或目的地不安全、遺失、重複、已封存或發生衝突時，completion 會失敗且不搬任何計畫；completion 狀態寫入失敗時也會回復已搬移的文件。Checkpoint、blocked 與未完成狀態不會觸發封存。
...
When the current task owns plan documents, the draft may include an optional `## Plan files` section containing their repo-relative paths as Markdown bullets. `handoff complete` archives only plans explicitly listed in that HANDOFF; it never scans directories or cleans up unlisted documents. General plans move to a sibling `archive/<year>/`, while plans under `.ai/plans/` move to `.ai/archive/plans/<year>/`. Unsafe, missing, duplicate, already archived, or conflicting paths block completion before any plan moves. Multi-file archival is all-or-nothing, and moved plans are restored if writing completed state fails. Checkpoint, blocked, and unfinished states never archive plans.
