...
When the current task has plan documents, list only those plans under `## Plan files` in the handoff. During `handoff complete`, archive only the listed plans. Never scan directories for unrelated plans.
